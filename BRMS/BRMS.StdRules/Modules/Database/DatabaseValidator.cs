    ...
    protected override async Task<IRuleResult> Execute(BRMSExecutionContext context, CancellationToken cancellationToken)
    {
        using (Logger.BeginScope(LogContext(context)))
        {
            ...
            try
            {
                ...
                object? value = context.NewValue?.GetValueAs<object>(PropertyPath);
                ...
                Logger.LogDebug("Ejecutando consulta SQL: {Query} para PropertyPath {PropertyPath}", processedSQL, PropertyPath);
                ...
                if (isValid)
                {
                    Logger.LogDebug("Validación de base de datos exitosa para PropertyPath {PropertyPath}", PropertyPath);
                ...
                else
                {
                    ...
                    Logger.LogInformation("Validación de base de datos falló para PropertyPath {PropertyPath}: {Value}", PropertyPath, value);
                    result = new RuleResult(this, context, errorMessage);
                }
